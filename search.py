...
for i in oth_matches:
	if inte%100000 == 0:print(str(inte)+" (min) "+str((t.time()-start)/60))
	all_cms.append(i[0])
	try:
...
for i in oth_matches:#change line thickness
	if i[2] == "IND" and len(i[6]) <= 1:
		...
		try:
			if i[4] == "":
				net.add_edge(i[0], i[6], label="$"+i[5], color="#000000")
			elif i[4][0] == "C":
				net.add_edge(i[0], i[4], label="$"+i[5], color="#000000")
			elif i[4][0] != "C" and len(i[6]) > 8:
				net.add_edge(i[0], i[6].replace("\n",""), label="$"+i[5], color="#000000")
			elif i[4][0] != "C" and i[6] == "\n":
				net.add_edge(i[0], "UNKNOWN", label="$"+i[5], color="#000000")
